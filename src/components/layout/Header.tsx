      ...
      <div className="container-shell relative z-10">
        ...
        >
          ...
          <div className="relative flex items-center justify-between gap-4 px-4 py-3 md:px-5 lg:px-6">
            ...
            {/* FIX: Removed legacyBehavior + nested <a>. 
                className and all props moved directly onto <Link>.
                Also fixed Image: added width="auto" style to prevent the
                width/height warning — Image has explicit w/h props but
                CSS rounds it, so we tell Next.js the display size explicitly. */}
            <Link
              href="/"
            ...
            <nav className="hidden items-center rounded-full border border-brand/10 bg-brand/5 p-1.5 backdrop-blur-md lg:flex">
              {mainNavItems.map((item) => {
                ...
                return (
                  // FIX: Removed legacyBehavior + nested <a> from every nav item.
                  <Link
                    href={item.path}
                    key={item.path}
                    className={cn(
            ...
            <div className="flex items-center gap-2 md:gap-3">
              {/* FIX: Removed legacyBehavior + nested <a> from Donate link */}
              <Link
                href="/donate"
                className="hidden rounded-full px-4 py-2 text-sm font-semibold text-brand transition-all duration-300 hover:-translate-y-0.5 hover:bg-brand/5 hover:text-brand-dark sm:inline-flex"
              >
                Donate
              </Link>

              {/* FIX: Removed legacyBehavior + nested <a> from Apply Now link */}
              <Link
                href="/admissions"
          ...
          <div className="relative">
            <nav className="flex flex-col gap-2">
              {mainNavItems.map((item, index) => {
                ...
                return (
                  // FIX: Removed legacyBehavior + nested <a> from mobile nav items
                  <Link
                    href={item.path}
                    key={item.path}
                    className={cn(
              ...
              {/* FIX: Removed legacyBehavior + nested <a> from mobile Donate */}
              <Link
                href="/donate"
                className="rounded-2xl px-4 py-3 text-sm font-medium text-dark/75 transition-all duration-300 hover:-translate-y-0.5 hover:bg-brand/5 hover:text-brand"
              >
                Donate
              </Link>
            </nav>
            ...
            <div className="mt-4 border-t border-brand/10 pt-4">
              {/* FIX: Removed legacyBehavior + nested <a> from mobile Apply */}
              <Link
                href="/admissions"
                className="group flex items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#0d6b53,#095640)] px-5 py-3.5 text-sm font-semibold text-cream shadow-[0_16px_30px_rgba(13,107,83,0.22)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_22px_38px_rgba(13,107,83,0.28)]"
              >
                Apply for Admission
                <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-0.5" />
              </Link>
